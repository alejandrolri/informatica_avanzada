...
void MainWindow::historia()
{
    ...
    if(avanzar==3 && atacar==1){
        ...
        ui->Atacar->setEnabled(true);
        ui->Huir->setEnabled(true);
        ui->textEdit->setText("Justo después te encuentras con un slime gigante bastante inofensivo. Decides atacar. Dichosos bichos, están por todo el reino. Descubres que tanto el camino del ogro como el del pasaje cercano conducen a la entrada de una mazmorra con 3 puertas. ¿Qué puerta escoges?");
        botones_puertas();
    }
    ...
    //Puerta 3
    if((atacar==0 && huir==2)|| (atacar==2 && huir==1))
    {
        mostrarImagen(2);
